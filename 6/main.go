...
func puzzle1(input string) int {
	...
	for _, instruct := range strings.Split(input, "\n") {
		i := parse(instruct)

		if i.isToggle {
			grid.toggle(i.minX, i.minY, i.maxX, i.maxY)
		} else {
			grid.turn(i.minX, i.minY, i.maxX, i.maxY, i.value, 1)
		}
	}
...
func puzzle2(input string) int {
	...
	for _, instruct := range strings.Split(input, "\n") {
		...
		grid.turn(i.minX, i.minY, i.maxX, i.maxY, i.value, math.MaxInt32)
	}

	return grid.brightness()
}

func parse(i string) instruction {
	i = strings.Replace(i, "turn on", "turnon", 1)
	i = strings.Replace(i, "turn off", "turnoff", 1)

	instruct := strings.Fields(i)
	min := strings.Split(instruct[1], ",")
	max := strings.Split(instruct[3], ",")

	minX, _ := strconv.Atoi(min[0])
	minY, _ := strconv.Atoi(min[1])
	maxX, _ := strconv.Atoi(max[0])
	maxY, _ := strconv.Atoi(max[1])

	value := -1
	if instruct[0] == "turnon" {
		value = 1
	}
...
func (l *lightgrid) toggle(minX, minY, maxX, maxY int) {
	for x := minX; x <= maxX; x++ {
		for y := minY; y <= maxY; y++ {
			l.lights[x][y] = -(l.lights[x][y] - 1)
		}
	}
}

func (l *lightgrid) turn(minX, minY, maxX, maxY int, value, maxValue int) {
	for x := minX; x <= maxX; x++ {
		for y := minY; y <= maxY; y++ {
			l.lights[x][y] = int(math.Min(math.Max(float64(l.lights[x][y]+value), 0.0), float64(maxValue)))
		}
	}
}

func (l *lightgrid) brightness() int {
	b := 0
	for x := 0; x < len(l.lights); x++ {
		for y := 0; y < len(l.lights[x]); y++ {
			b += l.lights[x][y]
		}
	}
	return b
