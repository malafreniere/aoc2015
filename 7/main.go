...
type expression interface {
	value(mem map[string]expression) uint16
}

type evaluable interface {
	eval(mem map[string]expression)
}

type null struct {
}

func (v null) value(mem map[string]expression) uint16 {
	panic(fmt.Errorf("null expression"))
}

type val uint16

func (v val) value(mem map[string]expression) uint16 {
	return uint16(v)
}
...
func (a assign) eval(mem map[string]expression) {
	mem[a.right.string()] = a.left
}
...
func (v variable) value(mem map[string]expression) uint16 {
	value := mem[v.string()].value(mem)
	mem[v.string()] = val(value)
	return value
}
...
func (n not) value(mem map[string]expression) uint16 {
	v := n.right.value(mem)
	return ^v
}
...
func (a and) value(mem map[string]expression) uint16 {
	return a.left.value(mem) & a.right.value(mem)
}
...
func (a or) value(mem map[string]expression) uint16 {
	return a.left.value(mem) | a.right.value(mem)
}
...
func (l lshift) value(mem map[string]expression) uint16 {
	return l.left.value(mem) << uint16(l.right.value(mem))
}
...
func main() {
	...
	a = mem["a"].value(mem)
	fmt.Printf("Puzzle 2: a := %v\n", a)

}

func parse(left expression, tokens []string) evaluable {
	for i := 0; i < len(tokens); i++ {
		...
		return parse(variable(tokens[i]), tokens[i+1:])
	}
