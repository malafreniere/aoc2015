...
func isNiceWord2(word string) bool {
	...
	for i := 0; i < len(word)-1; i++ {
		if i+2 < len(word) && word[i] == word[i+2] {
			between = true
		}

		pair := fmt.Sprintf("%v%v", word[i], word[i+1])
		if v, ok := pairs[pair]; ok && v != i-1 {
			hasPair = true
		} else if ok {
			return false
		} else {
	...
	return between && hasPair
}

func isNiceWord1(word string) bool {
	vowels := 0
	twice := false
	prev := ' '
